...
/* cl -Fe: qxltool.exe -INT -D__WINNT=1 /O2 qxltool.c NT/getopt.c */
...
extern int fnmatch (char *p, char *s);
# endif
...
uint32_t inline swaplong (uint32_t val)
{
...
static long d_list (QXL * qxl, QLDIR * d, void *a, void *pb, u_short c)
{
    struct tm *tm;
    char s[128];
    size_t n;
    ...
    if(flen >=0)
    {
        ...
        if (d->type == 1)
        {
            // TBD sprintf (s + n, "(%ld) ", d->data);
            sprintf (s + n, "(%u) ", d->data);
        }
        fputs (s, qxl->fp);
        fputc ('\n', qxl->fp);
    }
...
static long qfindwild (QXL * qxl, QLDIR * d, void *fn, void *pft, u_short flag)
{
    QLDIR *pd = NULL;
...
static long qfindbest (QXL * qxl, QLDIR * d, void *fn, void *pft, u_short flag)
{
...
static long qmatch (QXL * qxl, QLDIR * d, void *fn, void *pft, u_short flag)
{
    QLDIR *pd = NULL;
...
static long processdir (QXL * qxl, QLDIR * buf, QLDIR * cur,
...
static int qinfo (QXL * qxl, short mflag, char **p)
{
    ...
    fprintf (qxl->fp, "Formatted %d, (map len %d*512), rand %d, access %d\n",
	     qxl->h.formatted, qxl->h.formatted, qxl->h.randd, qxl->h.access);
    return 0;
}
...
static int qwrite (QXL * qxl, short mflag, char **av)
{
    ...
    if ((fn0 = *av) != NULL)
    {
		...
		if (fp != NULL)
		{
		    ...
		    fstat (fileno (fp), &st);
                    need = ((sizeof (QLDIR) + (int32_t)st.st_size) - dlen) /
                        qxl->h.sectc;
			...
			fprintf (stderr,
                                 ...
                                 fn0, qxl->h.sectc,
                                 qxl->h.free,
                                 (int32_t) st.st_size + sizeof (QLDIR),
                                 need
                                 );
		    }
		    else
...
static long clone_cb (QXL * sqxl, QLDIR * d, void *q, void *pb, u_short c)
{
    QXL *dqxl = (QXL *)q;
    int flen;
    char *sd, *dd;
